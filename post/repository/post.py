...
def get_all_tab(db: Session):
    stmt = select(models.Post).options(joinedload(models.Post.comments),
                                       joinedload(models.Post.comments, models.Comment.creator))
...
def update(id: int, request: schemas.Post, db: Session):
    ...
    return HTTPException(status_code=status.HTTP_202_ACCEPTED,
                         detail=f"post with id {id} was updated with title:{request.title} and text:{request.text}",
                         headers=[])
